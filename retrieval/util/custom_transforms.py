...
class Resize(object):
    ...
    def __call__(self, imgs):
        ...
        resize_imgs = []
        for img in imgs:
            label_resize_img = F.resize(img, self.size, self.interpolation)
...
class RandomCrop(object):
    ...
    def __call__(self, imgs):
        ...
        for img in imgs:
            ...
            paded_imgs.append(img)

        i, j, h, w = self.get_params(paded_imgs[0], self.size)
        croped_imgs = []
        for img in paded_imgs:
            croped_imgs.append(F.crop(img, i, j, h, w))
        return croped_imgs
